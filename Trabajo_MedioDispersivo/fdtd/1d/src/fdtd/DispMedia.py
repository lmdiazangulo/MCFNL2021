...
class SolverComplete:
    ...
    def __init__(self, mesh, options, probes, sources, data):
        ...
        e[:] = SolverComplete._gaussian(x[:], data["InitialPulse"]["Amp"], data["InitialPulse"]["Mean"], data["InitialPulse"]["Std"])
    ...
    def _betap(self, coord):
        dt=self._dt()
        a=np.multiply(self.k,self.data['ap'])
        c=np.multiply(self.k,self.data['cp'])
    ...
    def _updateE(self, t, dt):
        ...
        eNew[1:-1] = c1 * e[1:-1] + \
            2 * dt * ( h[1:] - h[:-1] - np.real( Suma[1:-1] ) ) / \
                    (2 * sp.epsilon_0 * self.data['medio']['epsinf'] + 2 * Rvbetap + sigma * dt)
        
        for i in range(self.l):
            jNew[i,1:-1] = self._kp(i) * j[i,1:-1] + self._betap(i) * ( eNew[1:-1] - e[1:-1] ) / dt
        ...
        return (c1,Rvbetap,Rvkp,Suma,len(e))
