...
with open('Am_con_Al.txt') as file_object:
    Am_Al = file_object.read()
    Am_Al = Am_Al.split('\n')
    
    while '' in Am_Al:   #removal of empty lines
        Am_Al.remove('')
    
    Am_Al = list(map(int, Am_Al))       #converting it to numbers
        
with open('americioAireNopol.txt') as file_object:
    Am_mal = file_object.read()
    Am_mal = Am_mal.split('\n')   
    
    while '' in Am_mal:   #removal of empty lines
        Am_mal.remove('')
    
    Am_mal = list(map(int, Am_mal))       #converting it to numbers    

with open('americioVacio_pol.txt') as file_object:
    Am = file_object.read()
    Am = Am.split('\n')   
    
    while '' in Am:   #removal of empty lines
        Am.remove('')
    
    Am = list(map(int, Am))       #converting it to numbers  
    
with open('muestra_problema_aire_no_pol.txt') as file_object:
    problema_mal = file_object.read()
    problema_mal = problema_mal.split('\n')   
    
    while '' in problema_mal:   #removal of empty lines
        problema_mal.remove('')
    
    problema_mal= list(map(int, problema_mal))       #converting it to numbers
    
with open("muestra_problema.txt") as file_object:
    problema = file_object.read()
    problema = problema.split('\n')
    
    while '' in problema:   #removal of empty lines
        problema.remove('')
    
    problema = list(map(int, problema))       #converting it to numbers  
    
with open('pulser.txt') as file_object:
    pulser = file_object.read()
    pulser = pulser.split('\n')
    
    while '' in pulser:   #removal of empty lines
        pulser.remove('')
...
###################1.1) Representacion###################
    ...
    #i) Plot normal
import matplotlib                   #la libreria clave pa pintar
